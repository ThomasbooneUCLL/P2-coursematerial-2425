...
def count_movies_from_current_year(movies,year):
    ...
    for movie in movies:
        if movie.year == year:
            counter += 1
    return counter
def select_movies_with_tom_hanks(movies,ActorName):
    result_movies = []
    for movie in movies:
        if ActorName in movies.actors:
            result_movies.append(movie)
    return result_movies
def sum_movie_duration_in_90s(movies,beginYear,Endyear):
    total_duration = 0
    for movie in movies:
        if movie.year >= beginYear and movie.year < Endyear:
            total_duration += movie.duration
    return total_duration
def get_directors_of_tom_hanks(movies,NameActor):
    directors = []
    for movie in movies:
        if NameActor in movie.actors:
